...
const CONFIG = {
  ...
  // 進件只等可靠建單回覆；排盤已改在回應後繼續。這是共用 API 的網路故障上限。
  timeoutMs: 60000,
  ...
  PRICE: {
    ...
    // 訂製線的物流。2026-08-05 使用者要求：訂製也要能選宅配到府。
    // ⚠ 金額與下面 SHOP.shipping 是同一組真值的兩份拷貝（訂製走 pps-intake、
    //    現貨走 pps2-shop-order，是兩條獨立管線）。改一邊務必兩邊都改，
    //    後端 pps2_settings.shipping_rules 也要跟（見 SHOP.shipping 的註解）。
    shipStore: 60,    // 物流：店到店（超商取貨）
    shipHome: 130,    // 物流：宅配到府
    upgradeLow: 150,  // 升等推薦（老闆單選其一，裁決§0-3）
    upgradeHigh: 300
  },

  // 訂製線的付款方式（2026-08-05 使用者要求：要有現金）。
  // 訂製是「問卷 → 報價單 → 付款」，這裡收的是客戶的偏好，正式收款仍以報價單為準。
  CUSTOM_PAY: [
    { id:"transfer", label:"銀行匯款" },
    { id:"line",     label:"LINE 轉帳" },
    { id:"cash",     label:"現金（面交當場付）" }
  ],
  // 訂單進度七階段
